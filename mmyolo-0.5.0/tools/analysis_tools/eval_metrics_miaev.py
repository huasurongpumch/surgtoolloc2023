...
def main():
    ...
    print(eval_results)
    with open('/data1/surgtoolloc/mmyolo_0.5.0/class.txt', 'r') as fp:
        classes = fp.readlines()
    with open('self_tool2-0.1_0.65.txt','w') as fp:
        for class1 in classes:
            class1 = class1.strip()
            for key,values in eval_results.items():
                if key == class1+'/recall':
                    fp.write(key + '\t' + str(values) +'\t')
                if key == class1+'/precision':
                    fp.write(key + '\t' +str(values))
                    fp.write('\n')
